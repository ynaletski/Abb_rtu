...
const unsigned char size_conf[8]={116,116,116,116,240,0,216,236};
const unsigned char etl[8] = {1,2,4,8,16,32,64,128};
const unsigned char ind_prm_alm[8]={26,27,29,30,32,33,24,25};

/*0-неисп,1-коммуник,2-MVS/ABB,3-DCON,4-Mogbus,5-Micromotion*/
...
const unsigned char dyn_set[Max_dyn_massive][9]={{0,0,1,2,0,2,66,1,0},
 ...
 {24,1,48,18,2,0,66,1,1},{24,2,48,18,2,0,66,1,1},{24,3,48,18,2,0,66,1,1},
 {24,4,48,18,2,0,66,1,1},{24,5,48,18,2,0,66,1,1},{24,6,48,18,2,0,66,0,1},
 ...
 {41,0,30,0,0,1,1,0,0},{42,0,49,8,0,1,1,0,0},{43,0,69,18,0,0,66,0,0}};
...
unsigned char main_select[Max_select_main]={
  10,2,3,0,51,52,  11,2,53,54,  13,6,55,56,57,58,35,36,
  14,6,55,56,57,58,35,36, 15,6,55,56,57,58,35,36, 16,6,55,56,57,58,35,36,
  12,2,0,51, 67,2,87,88, 68,2,0,1, 69,2,0,1};
...
unsigned char conf_basic[Max_conf][7]={
 ...
 {9/*Межповер инт(11)*/,4,12,4,128,22,1},
 {11/*Диаметр трубы(12)*/,4,16,4,128,1,1},{12/*Диам сенсора(13)*/,4,20,4,128,1,1},
 {13/*Отсечка (14)*/,4,24,4,128,2,1},{16/*Радиус кромки(15)*/,4,28,4,128,1,1},
 {17/*Шероховатость(16)*/,4,32,4,128,1,1},
 {69/*Плотность (17)*/,4,36,4,128,8,1},{70/*Азот  (18)*/,4,40,4,128,20,1},
 {71/*Углек.газ(19)*/,4,44,4,128,20,1},
 {14/*Верхняя граница(20)*/,4,48,4,128,2,1},{15/*Нижняя гран(21)*/,4,52,4,128,2,1},
 {26/*Ниж алм пер(22)*/,4,56,4,128,2,1},
 {27/*Верх алм перепад(23)*/,4,60,4,128,2,1},{28/*Гист перепад(24)*/,4,64,4,128,2,1},
 {29/*Ниж алм давл(25)*/,4,68,4,128,2,1},{30/*Верх алм давл(26)*/,4,72,4,128,2,1},
 {31/*Гистер давл(27)*/,4,76,4,128,2,1},{32/*Ниж алм темп(28)*/,4,80,4,128,3,1},
 {33/*Верх алм темп(29)*/,4,84,4,128,3,1},{34/*Гистерез тем(30)*/,4,86,4,128,3,1},
 {24/*Миним расход(31)*/,4,92,4,128,17,1},{25/*Макс расход (32)*/,4,96,4,128,17,1},
 {76/*Гистер расход (33)*/,4,100,4,128,17,1},{35/*Тег идент(34)*/,10,104,10,128,0,2},
 {63/*Корр мин.расх(35)*/,1,114,2,128,0,2},{7/*Статус алармов(36)*/,1,115,3,128,0,2},
};
...
unsigned char conf_select[Max_select]={
  ...
  6,2,59,60, 36,4,45,46,47,48, 35,2,0,1,
...
void InitBasicStruct (unsigned char num,struct configparam *cnf,
		      struct dynparams *bs)
{
  unsigned char i,j,k,cr[4];float buf[23];
  ...
  cnf->enable_alm=ReadEEP(num,115);
  cnf->therm_corr=ReadEEP(num,6);cnf->flow_corr=ReadEEP(num,114);
  ...
  { /*получение основных конфиг.параметров*/
    j=i*4;
    ConvToFloatVerify(&buf[i],ReadEEP(num,12+j),ReadEEP(num,13+j),
		      ReadEEP(num,14+j),ReadEEP(num,15+j));
  ...
  cnf->hi_stack=buf[9];cnf->lo_stack=buf[10];
  for (i=0;i<4;i++) for (j=0;j<3;j++) cnf->borders[i][j]=buf[11+3*i+j];
  for (i=0;i<10;i++) cnf->tag[i]=ReadEEP(num,104+i);
}
...
unsigned char CalcFlowMain (struct configparam Prm,struct dynparams *Bs)
{
   ...
   unsigned char result,step;result=0;Bs->dyn[24]=0.0;
   if ((Prm.pipe_size<=0)||(Prm.sens_size<=0)||(Prm.m_pipe>11)||
       (Prm.m_sens>11)||(Prm.sensor>10)) {result=1;goto M;}
   if ((Bs->dyn[12]<0.6)||
       (Bs->dyn[12]>1.5)||(Bs->dyn[16]>10)||(Bs->dyn[20]>5)) {result=2;goto M;}
   if ((Bs->dyn[4]<=0)||(Bs->dyn[8]< -50)||(Bs->dyn[4]>5500)||
       (Bs->dyn[8]>50)||(Bs->dyn[0]>63)) {result=4;goto M;}
   ...
   if (VerifyAnnubar (Prm.sensor-3,& Prm.sens_size)==1) result=8;/*проверка ширины аннубара*/
   ...
   if (Prm.method==0)
   { if (CalcNX19Koeff(Pp,Tp,pmega,tabs,std,&Z)!=0) result=(result & 15)+16;}/*проверка*/
   else
   {
   ...
   if (Prm.sensor<3 && Bs->dyn[0]>=Prm.cut_flow && Bs->dyn[12]>0)
   {
     ...
     if ((B > 0.75)||(B < 0.2)) {result=8;goto M;}

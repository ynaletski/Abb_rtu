...
/*Дата последнего изменения: 14-01-2011, вместо ASCII  RTU          */
/*01.02.2021 Abb instead MVS, снято огарничение на плотность        */
/*(Bs->dyn[12]>1.5) для попутного газа  ГОСТ 137 и коэфициент       */
/*сжимаемости должен быть Z = 1                                     */
/********************************************************************/
#include<mem.h>
#include<7188xa.h>
#include<X607.h>
...
unsigned char
  ...
  avg_old,dlt_tm,cnt_cbr,mmi_flg_ver,icp_pool,flg_pool,
  //01.02.2021
  cnt_abb,cnt_abb_old;
...
void RestoreSetAlarmsAIN (unsigned char num);
void WriteAlarmsSetup (unsigned char buf_com[]);
void SetClearAlarmsMVS (unsigned char num_pnt);
...
void SendToABB (unsigned char number);
void ReadFromABB (unsigned char number, unsigned char buf[]);
void GetDataABB(unsigned char buf[]);
...
void AverageBasicParam (unsigned char num);
...
unsigned char DataToModbus (unsigned adr_reg,unsigned *cnt_reg,
	      unsigned char data[],unsigned func,unsigned char num);
...
void GetModuleAin (unsigned char buf_com[],unsigned char num);
void GetModuleAout (unsigned char buf_com[],unsigned char num);
...
void SaveParameters (unsigned char num);
...
void main (void)
{
  unsigned char i,j,k,cr[10],buf_evt[72];double param;/*float val_proba;*/
  /*DisableWDT();*/
  InitLib();X607_Init();
  ...
  for (;;)
  {
    /*ConvToBynare(100.12345,cr);ConvToFloatVerify(&val_proba,cr[0],cr[1],cr[2],cr[3]);
    printf(" %f ",val_proba);*/
    /*printf(" begin ");
    /*printf(" %d ",Port[1].status);*/
    /*printf(" %f ",Basic[0].dyn[0]);printf(" %f ",Basic[0].dyn[1]);
    printf(" %f ",Basic[0].dyn[2]);printf(" %f ",Basic[0].dyn[3]);*/
    ...
    if (Port[1].status == 4) /* обработка ответа от MVS */
    {
      ...
      if (typ_pool < 13) //was <5
      {
        ...
        if (Port[1].buf[Port[1].index-1]==cr[0] && Port[1].buf[Port[1].index-2]==cr[1])
          ReadFromABB(Prt.nmb,Port[1].buf);  
      } 
	...
	Host.num_pool++;
     ...
     { /*периодическое раз в секунду сохранение значений даты и времени*/
       ...
       if (dlt_tm >=3)
       {
	 avg_old=sec;for (i=0;i<Max_pnt;i++) Config[i].flag=1;Prt.flg_err=1;
	 for (k=0;k<Max_pnt;k++) for (i=0;i<Max_dyn;i++)
	 if (main_dyn[i][2] == 3) for (j=0;j<2;j++)
	 Basic[k].dyn[main_dyn[i][0]+j+1]=(Basic[k].dyn[main_dyn[i][0]+j+1]*
			 cnt[j]+Basic[k].dyn[main_dyn[i][0]])/(cnt[j]+1);
	 AverageExpandParams(); for (j=0;j<2;j++) cnt[j]++;
       }
	 ...
	 SetClearAlarms(&Config[i],Basic[i],i); AverageBasicParam(i);
       } AverageMinutExpandParam(); cnt[2]++;
       for (i=0;i < Max_mvs;i++) SetClearAlarmsMVS(i);
	...
	for (i=0;i<Max_dyn;i++) if (main_dyn[i][2] == 2)
	{
	  for (j=0;j<4;j++) Basic[k].dyn[main_dyn[i][0]+ind_accum[j]]=
	    ...
	    Basic[k].dyn[main_dyn[i][0]+7]; Basic[k].dyn[main_dyn[i][0]+7]=0.0;
	}
	for (i=0;i<Real_exp_dyn;i++) if (exp_dyn[i][2] == 2)
	{
      ...
      for (k=0;k< Max_pnt;k++) SaveParameters(k);SaveExpParams();
	...
	for (i=0;i< Max_pnt;i++)
	{
	  ...
	  { /*сохранение расхода за прошлый час*/
	    Basic[i].dyn[43]=Basic[i].dyn[main_dyn[j][0]+1];
	    Basic[i].dyn[main_dyn[j][0]+1]=0.0;
	  }
	}
	for (i=0;i<Real_exp_dyn;i++) if (exp_dyn[i][2] == 2)
	Expand.dyn[exp_dyn[i][0]+1]=0.0; cnt[1]=0;
...
void MyTimer (void)
{
   ...
   //01.02.2021 for (a=0;a<Max_mvs;a++) if (Sensor[a].wait != 0) cnt_cbr++;
   ...
   if (Port[3].status==1)
   {
    ...
    if (typ_pool != 0)
    {
      ...
      if (Port[1].timer > Port[1].ta)
      {
        ...
        typ_pool=0;
        ClearCom(2);
	      //01.02.2021 if (Prt.nmb < 4) Sensor[Prt.nmb].setup[2]=128;/*MVS не отвечает*/
      ...
      if (Port[1].status==1)
      {
        if (cnt_abb!=cnt_abb_old) cnt_abb_old=cnt_abb; 
        else
        {
          Port[1].status=4; 
          Port[1].timer=0; 
   ...
   if (Port[2].status==1)
   {
	...
	{ SetStatusModbus(8);Host.num_pool++; }
     ...
     if (Modbus.protocol == 0) /*протокол ASCII*/
     {
...
unsigned char SetExpandDescript (void)
{
  ...
  unsigned char i,j,k,n,cr[4],flag;unsigned count;
  count=1;j=n=flag=0;
...
void InitModuleStruct (unsigned char num)
{
  ...
  for (i=0;i<8;i++)
  {
    ...
    ConvToFloatVerify(&Ain[num].hi_brd[i],ReadEEP(6,80+j),
	   ReadEEP(6,81+j),ReadEEP(6,82+j),ReadEEP(6,83+j));
...
void InitModuleOutStruct (unsigned char num)
{
  ...
  for (i=0;i<4;i++)
  {
    ...
    ConvToFloatVerify(&Aout[num].hi_brd[i],ReadEEP(6,184+j),
...
void RestoreExpandParameters (void)
{
  ...
  for (i=0;i<Real_exp_dyn;i++) if (exp_dyn[i][2]>0 && exp_dyn[i][2]<4)
  {
    for (j=0;j<exp_dyn[i][1];j++)
    {
      ...
      ConvToFloatVerify(&Expand.dyn[exp_dyn[i][0]+j],buf[0],
			buf[1],buf[2],buf[3]);ind++;
    }
    if (exp_dyn[i][2]==2)
    { /*восстановление минутных приращений*/
...
void RestoreSetAlarmsMVS (unsigned char num)
{
  ...
  X607_ReadFn(0xc000+num*64,40,buf);
  for (i=0;i<3;i++) for (j=0;j<3;j++)
  ConvToFloatVerify(&Sensor[num].alm_set[i][j],buf[j*4+i*12],
	      buf[1+j*4+i*12],buf[2+j*4+i*12],buf[3+j*4+i*12]);
  Sensor[num].alm_enb=buf[36];
  X607_ReadFn(0xc330+num*4,4,buf);
  for (i=0;i<3;i++) Sensor[num].alm_status[i]=buf[i];
}
/***** восстановление уставок алармов для аналог входов *********/
void RestoreSetAlarmsAIN (unsigned char num)
{
  unsigned char i,j,buf[170];
  X607_ReadFn(0xc100+num*256,170,buf);
  for (i=0;i<8;i++)
  {
    for (j=0;j<5;j++)
    ConvToFloatVerify(&Ain[num].alm_set[i][j],buf[j*4+i*20],
		buf[1+j*4+i*20],buf[2+j*4+i*20],buf[3+j*4+i*20]);
    Ain[num].alm_status[i]=buf[i+160];
  } Ain[num].alm_enb=buf[168];
}
...
void SetClearAlarmsMVS (unsigned char num)
{
  unsigned char i;
  for (i=0;i<3;i++) if ((Sensor[num].alm_enb & etl[i])>0)
  Sensor[num].alm_status[i]=SetClearAlarmsPrm(Sensor[num].alm_set[i],
	   Sensor[num].alm_status[i],Sensor[num].avg[i],num,i);
}
...
/*	       count=21;typ_pool=3;break;
	case 3:for (i=0;i< 6;i++) bufs[3+i]=mvs_cbr[i];
	       bufs[9]=mvs_cmd[0];bufs[10]=mvs_cmd[1];
	       for (i=0;i< 4;i++) bufs[11+i]=mvs_val[i];
...
void SendToABB (unsigned char number)
{
  ...
  if (Device.adr_mvs[number]>0)
  {
    ...
    switch (Sensor[number].evt)
    {
      case 0: /*400 (401-DP) переменные*/
        bufs[1]=4; bufs[2]=1; bufs[3]=144; bufs[4]=0; bufs[5]=2; 
        count=6; typ_pool=1;
      break;

      case 1: /*402 (403-P) переменные*/
        bufs[1]=4; bufs[2]=1; bufs[3]=146; bufs[4]=0; bufs[5]=2; 
        count=6; typ_pool=2;
      break;
      
      case 2: /*405 (405-T) переменные*/
        bufs[1]=4; bufs[2]=1; bufs[3]=148; bufs[4]=0; bufs[5]=2; 
        count=6; typ_pool=3;
      break;

      case 3: /*440 (441-UpDP) границы калиб.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=184; bufs[4]=0;bufs[5]=2; 
        count=6; typ_pool=4;
      break;

      case 4: /*442 (443-LowDP) границы калиб.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=186; bufs[4]=0;bufs[5]=2; 
        count=6; typ_pool=5;
      break;

      case 5: /*444 (445-UpP) границы калиб.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=188; bufs[4]=0;bufs[5]=2; 
        count=6; typ_pool=6;
      break;

      case 6: /*446 (447-LowP) границы калиб.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=190; bufs[4]=0;bufs[5]=2; 
        count=6; typ_pool=7;
      break;

      case 7: /*450 (451-UpT) границы темпер.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=194; bufs[4]=0;bufs[5]=2; 
        count=6; typ_pool=8;
      break;

      case 8: /*452 (453-LowT) границы темпер.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=196; bufs[4]=0; bufs[5]=2; 
        count=6; typ_pool=9;
      break;

      case 9: /*449 (450-CalibrationUnitPressure) единицы изм.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=193; bufs[4]=0;bufs[5]=1; 
        count=6; typ_pool=10;
      break;

      case 10: /*455 (456-CalibrationUnitTemperature) единицы изм.*/
        bufs[1]=3; bufs[2]=1; bufs[3]=199; bufs[4]=0;bufs[5]=1; 
        count=6; typ_pool=11;
      break;

      case 11: /*статус ошибок*/
        bufs[1]=1; bufs[2]=0; bufs[3]=1; bufs[4]=0;bufs[5]=27; 
        count=6; typ_pool=12;
      break;
...
void ReadFromABB (unsigned char number, unsigned char buf[])
{
  ...
  switch (typ_pool)
  {
    ...
    case 4:
      ConvToFloatVerify (&Sensor[number].data[4], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=4; 
    break;

    case 5:
      ConvToFloatVerify (&Sensor[number].data[5], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=5; 
    break;

    case 6:
      ConvToFloatVerify (&Sensor[number].data[6], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=6; 
    break;
    
    case 7:
      ConvToFloatVerify (&Sensor[number].data[7], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=7; 
    break;

    case 8:
      ConvToFloatVerify (&Sensor[number].data[8], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=8; 
    break;
    
    case 9:
      ConvToFloatVerify (&Sensor[number].data[9], buf[num+3],buf[num+2], buf[num+1], buf[num]);
	    Sensor[number].evt=9; 
    break;

    case 10:
      Sensor[number].units[0]=buf[num]*256+buf[num+1];
	    Sensor[number].units[1]=Sensor[number].units[0];
	    Sensor[number].evt=10; 
    break;

    case 11:
      Sensor[number].units[2]=buf[num]*256+buf[num+1];
	    Sensor[number].evt=11; 
    break;

    case 12:
      if (buf[2]>0) for (i=0;i<buf[2];i++) Sensor[number].err[i]=buf[i+num]; 
      Sensor[number].evt=0; 
    break;
...
void AverageMinutExpandParam (void)
{
  ...
  { /*все минутные усреднения и накопления*/
    if (exp_dyn[i][2] == 3)
    Expand.dyn[exp_dyn[i][0]+3]=(Expand.dyn[exp_dyn[i][0]+3]*cnt[2]+
      Expand.dyn[exp_dyn[i][0]])/(cnt[2]+1);
    if (exp_dyn[i][2] == 2) Expand.dyn[exp_dyn[i][0]+7]=
      (Expand.dyn[exp_dyn[i][0]+7]*cnt[2]+Expand.dyn[exp_dyn[i][0]])/
		      (cnt[2]+1);
  } ind=0;
  for (i=0;i<Real_exp_dyn;i++) if (exp_dyn[i][2]>0 && exp_dyn[i][2]<4)
  for (j=0;j<exp_dyn[i][1];j++)
  { /*сохранение минутных приращений*/
    ...
    {
      ConvToBynare(Expand.dyn[exp_dyn[i][0]+j]/3600*cnt[2],cr);
...
void AverageExpandParams (void)
{
  ...
  Expand.dyn[exp_dyn[i][0]+j+1]=(Expand.dyn[main_dyn[i][0]+j+1]*
       cnt[j]+Expand.dyn[exp_dyn[i][0]])/(cnt[j]+1);
}
...
void ReadFromArchive (unsigned char bufer[])
{
  ...
  int pnt_arc;
  unsigned segf,adrf;
  num_page=bufer[7]; pnt_arc=bufer[8]*256+bufer[9];
  typ_arc=bufer[10]; bufer[11]=0;
  switch (num_page)
  {
  ...
  } bufer[7]=i; bufer[8]=num_page; bufer[9]=pnt_arc/256;
  bufer[10]=pnt_arc-bufer[9]*256; bufer[6]=16+i*Size_str;
}
...
void GetValuesScale (unsigned char num_scale,float *lo,float *hi)
{
  ...
  ConvToFloatVerify(lo,buf_val[0],buf_val[1],buf_val[2],buf_val[3]);
  ConvToFloatVerify(hi,buf_val[4],buf_val[5],buf_val[6],buf_val[7]);
}
	   ...
	   unsigned char data[],unsigned ind_func,unsigned char num)
{
  ...
  for (i=0;i<*cnt_reg;i++)
  {
    ...
    {
      value=0.0;
      if (ind_func==2) /*функция с номером 03*/
      {
	...
	switch (buf_func[5])
	{
      ...
      if (ind_func==7 && buf_func[2]==2) /*функция с номером 16 доп точка*/
      {
	switch (buf_func[5])
	{
...
void ModbusSlave (unsigned char buf[], unsigned char index)
{
  ...
  M1:data[0]=addr;data[1]=func+0x80;data[2]=mb_err;
     ...
     count=5; /*ответ с кодом ошибки*/
  M2:if (Modbus.connect == 2) ToComBufn_3(data,count);
     if (Modbus.connect == 3)
...
/** выдача запроса в Modbus-устройство,подключ к СОМ3 или СОМ1 ****/
void SendModbus (unsigned char buf[],unsigned char num_port)
{
  unsigned char data[7],buf_com[18],i;
...
void GetAllDescript (unsigned char buf[])
{
  ...
  switch (opt)
  {
    ...
    case 3:for (i=0;i < Max_dyn_massive;i++) for (j=0;j<4;j++)
	   buf[11+j+i*4]=dyn_set[i][j]; buf[9]=Max_dyn_massive;
    ...
    case 8:for (i=0;i < Max_archive;i++) for (j=0;j<4;j++)
	   buf[11+j+i*4]=set_archive[i][j]; buf[9]=Max_archive;buf[7]=4;
...
void GetModuleAin (unsigned char buf_com[],unsigned char num)
{
  ...
  if (buf_com[8] == 0)
  {   /*получение установок*/
    for (i=0;i<6;i++) buf_com[10+i]=Ain[num].status[i];
    for (i=0;i<8;i++)
    {
      ConvToBynare(Ain[num].lo_brd[i],cr);
      for (k=0;k< 4;k++) buf_com[16+i*4+k]=cr[k];
      ConvToBynare(Ain[num].hi_brd[i],cr);
      for (k=0;k< 4;k++) buf_com[48+i*4+k]=cr[k];
      buf_com[80+i]=Ain[num].units[i];
      for (j=0;j<5;j++)
      {
	ConvToBynare(Ain[num].alm_set[i][j],cr);
	for (k=0;k< 4;k++) buf_com[88+j*4+i*20+k]=cr[k];
      } buf_com[248]=Ain[num].alm_enb;
    } buf_com[6]=253;
  } else
  {  /*получение данных и статусов алармов*/
    for (i=0;i<8;i++)
    {
      ConvToBynare(Ain[num].prm[i],cr);
      for (k=0;k< 4;k++) buf_com[10+i*4+k]=cr[k];
      buf_com[42+i]=Ain[num].alm_status[i];
    } buf_com[6]=54;
  }
}
...
void GetModuleAout (unsigned char buf_com[],unsigned char num)
{
  ...
  for (i=0;i<6;i++) buf_com[10+i]=Aout[num].status[i];
  for (i=0;i<4;i++)
  {
    ConvToBynare(Aout[num].lo_brd[i],cr);
    for (k=0;k< 4;k++) buf_com[16+i*4+k]=cr[k];
    ConvToBynare(Aout[num].hi_brd[i],cr);
    for (k=0;k< 4;k++) buf_com[32+i*4+k]=cr[k];
    buf_com[48+i]=Aout[num].units[i];
  }
  for (i=0;i<4;i++)
  {
    ConvToBynare(Aout[num].prm[i],cr);
...
{
  unsigned char i;
  for (i=0;i<4;i++) buf_com[10+i]=Dio[num].status[i];
  buf_com[14]=Dio[num].inp; buf_com[15]=Dio[num].stat_out;buf_com[6]=20;
}
...
void GetDataABB(unsigned char buf_com[])
{
  unsigned char i, j, k, cr[4];
  if (buf_com[7]<4)
  {
	...
	if (buf[7]>0)
	{
      ...
      case 22:if (buf[7] < Max_icp)
		...
		{  /*тип,формат,имя модуля*/
		  case 0:GetModuleAin(buf,0);break;
		  case 1:GetModuleAin(buf,1);break;
		  case 2:GetModuleAout(buf,0);break;
		  case 3:GetModuleAout(buf,1);break;
		  case 4:GetModuleDio(buf,0);break;
		  case 5:GetModuleDio(buf,1);break;
...
void SaveParameters (unsigned char num)
{
  ...
  for (j=0;j<main_dyn[i][1];j++)
  {
    value=Basic[num].dyn[main_dyn[i][0]+j];ConvToBynare(value,cr);
    for (k=0;k< 4;k++) buf[ind*4+k]=cr[k];ind++;
  } X607_WriteFn(0x9000+num*Max_save,Max_save,buf);
}
